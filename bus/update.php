...
$name = $price = $discount = "";
$name_err = $price_err = $discount_err = "";
...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($input_discount)){
        $discount_err = "Please enter the discount amount.";
    } elseif(!ctype_digit($input_discount)){
    ...
    if(empty($name_err) && empty($price_err) && empty($discount_err)){
        // Prepare an insert statement
        $sql = "UPDATE employees SET name=?, price=?, discount=? WHERE id=?";

        if($stmt = mysqli_prepare($MySQLiconn, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssi", $param_name, $param_price, $param_discount, $param_id);
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <div class="form-group <?php echo (!empty($discount_err)) ? 'has-error' : ''; ?>">
